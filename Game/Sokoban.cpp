...
void Sokoban::reset(const std::string& filePath) {
    static sf::Texture down;
    if (!down.loadFromFile("player_05.png")) {
        exit(1);
    }
...
std::ifstream& operator>>(std::ifstream& in, Sokoban& sokoban) {
    in >> sokoban.h >> sokoban.w;
    sokoban.gameMatrix.resize(sokoban.h * sokoban.w);

    int playerX = -1, playerY = -1;
    for (int i = 0; i < sokoban.h * sokoban.w; ++i) {
        in >> sokoban.gameMatrix[i];
        if (sokoban.gameMatrix[i] == '@') {
            playerX = i % sokoban.w;
            playerY = i / sokoban.w;
        }
    }
    ...
    if (playerX != -1 && playerY != -1) {
        ...
        sokoban.player.setPosition(playerPos.x * 64, playerPos.y * 64);
    }
...
std::ostream& operator<<(std::ostream& out, const Sokoban& sokoban) {
    out << sokoban.h << " " << sokoban.w;
    return out;
}

int Sokoban::getArrayIndex(int x, int y) const {
    return x + y * w;
}

void Sokoban::convertToMatrixSpace(int i, int& x, int& y) const {
    x = i % w;
    y = i / w;
}
